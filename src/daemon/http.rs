use radio_tui::shared::protocol::Command;
...
pub fn start_server(
    ...
    tokio::spawn(async move {
        let app_state = AppState { state_manager, command_tx };
...
async fn stop(State(state): State<AppState>) -> StatusCode {
    info!("HTTP API: Stop");
    if state.command_tx.send(Command::Stop).await.is_err() {
...
async fn next_station(State(state): State<AppState>) -> StatusCode {
    ...
    if state.command_tx.send(Command::Next).await.is_err() {
...
async fn prev_station(State(state): State<AppState>) -> StatusCode {
    ...
    if state.command_tx.send(Command::Prev).await.is_err() {
...
async fn random_station(State(state): State<AppState>) -> StatusCode {
    ...
    if state.command_tx.send(Command::Random).await.is_err() {
