...
async fn test_fetch_and_parse_episode_metadata() {
    ...
    let url =
        "https://www.nts.live/shows/its-nation-time/episodes/its-nation-time-29th-january-2024";
    ...
    // 2. Fetch API data
    let api_data = api::fetch_episode(&show_name, &episode_alias)
        .await
        .unwrap();
    ...
    match write_metadata(&mp3_path, &metadata, image_data).await {
        Ok(_) => {
            ...
            match metadata::read_metadata(&mp3_path) {
                ...
                Err(e) => println!(
                    "Note: Could not read metadata back (file may be too small): {}",
                    e
                ),
            }
        }
        Err(e) => {
            ...
            println!(
                "Note: Metadata write failed (test file may be too small): {}",
                e
            );
        }
    }
...
fn test_url_parsing_edge_cases() {
    ...
    let url1 =
        "https://www.nts.live/shows/floating-points/episodes/floating-points-10th-october-2024";
...
async fn test_real_download() {
    ...
    let yt_dlp = download::find_yt_dlp().expect("yt-dlp required for this test");
    ...
    info!(
        "API data fetched - title: {}, broadcast: {}",
        api_data.name, api_data.broadcast
    );
    ...
    let audio_source = api::resolve_audio_source(&api_data, &metadata)
        .await
        .unwrap();
    ...
    metadata::write_metadata(&file_path, &metadata, image_data)
        .await
        .unwrap();
    ...
    match metadata::read_metadata(&file_path) {
        Ok(meta) => {
            ...
            assert!(
                meta.album == Some("NTS".to_string()),
                "Album should be 'NTS'"
            );
            assert!(meta.has_picture, "Should have artwork");

            info!(
                "Verification complete - title: {:?}, album: {:?}",
    ...
    if std::fs::create_dir_all(&keep_path).is_ok() {
        ...
        if std::fs::copy(&file_path, &dest).is_ok() {
            println!(
                "\n  (File also saved to: {} for inspection)",
                dest.display()
            );
        }
    }
