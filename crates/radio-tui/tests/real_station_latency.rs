...
impl StationResult {
    fn print(&self) {
        ...
        println!("│                                                             │");
        println!(
            "│  Time to First Byte (TTFB):      {:>8.2} ms           │",
            self.ttfb_ms
        );
        println!(
            "│  FFmpeg Decode Startup:          {:>8.2} ms           │",
            self.decode_startup_ms
        );
        println!("│  ────────────────────────────────────────────────────────  │");
        println!(
            "│  TOTAL STARTUP LATENCY:          {:>8.2} ms           │",
            self.total_startup_ms
        );
        println!("│                                                             │");
        println!(
            "│  Bytes received:                 {:>8} bytes          │",
            self.bytes_received
        );
        println!(
            "│  Test duration:                  {:>8.2} ms           │",
            self.duration_ms
        );
        println!(
            "│  Measured bitrate:               {:>8.2} kbps         │",
            self.measured_bitrate_kbps
        );
        println!(
            "│  Audio decoded:                   {:>8}               │",
            if self.audio_decoded {
                "✅ YES"
            } else {
                "❌ NO"
            }
...
async fn test_real_station_startup_latency() {
    ...
    for r in &results {
        let status = if r.error.is_some() {
            "❌"
        } else if r.audio_decoded {
            "✅"
        } else {
            "⚠️"
        };
        println!(
            "║  {:<25} │ {:>6.0}ms │ {:>6.0}ms │ {:>6.0}ms │ {:>6.0}k  {} ║",
            truncate(&r.name, 25),
            r.ttfb_ms,
            r.decode_startup_ms,
            r.total_startup_ms,
    ...
    if !successful.is_empty() {
        ...
        let avg_decode =
            successful.iter().map(|r| r.decode_startup_ms).sum::<f64>() / successful.len() as f64;
        let avg_total =
            successful.iter().map(|r| r.total_startup_ms).sum::<f64>() / successful.len() as f64;
        ...
        let fastest = successful
            .iter()
            .min_by(|a, b| a.total_startup_ms.partial_cmp(&b.total_startup_ms).unwrap())
            .unwrap();
        println!(
            "\n🏆 FASTEST: {} ({:.0} ms total)",
            fastest.name, fastest.total_startup_ms
        );

        // Find slowest decode (where optimization would help most)
        let slowest_decode = successful
            .iter()
            .max_by(|a, b| {
                a.decode_startup_ms
                    .partial_cmp(&b.decode_startup_ms)
                    .unwrap()
            })
            .unwrap();
        println!(
            "🐢 SLOWEST DECODE: {} ({:.0} ms)",
            slowest_decode.name, slowest_decode.decode_startup_ms
        );
    }
    ...
    let very_slow_connections = successful
        .iter()
        .filter(|r| r.total_startup_ms > 3000.0)
        .count();
    assert!(
...
async fn test_station_bitrate_stability() {
    ...
    for station in TEST_STATIONS {
        ...
        if bytes_per_second.len() >= 2 {
            let kbps_samples: Vec<f64> = bytes_per_second
                .iter()
...
async fn test_ffmpeg_probe_optimization() {
    ...
    let probe_settings = vec![
        (
            "Default",
            vec!["-probesize", "64k", "-analyzeduration", "200000"],
        ),
        (
            "Fast",
            vec!["-probesize", "32k", "-analyzeduration", "50000"],
        ),
        (
            "Minimal",
            vec!["-probesize", "16k", "-analyzeduration", "0"],
        ),
        (
            "Max",
            vec!["-probesize", "256k", "-analyzeduration", "500000"],
        ),
    ];
