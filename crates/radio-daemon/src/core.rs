/// DaemonCore — single-owner event loop for all mutable daemon state.
///
/// All tasks that previously held `Arc<Mutex<MpvController>>` now send
/// `DaemonEvent` messages to this loop.  DaemonCore owns `DaemonState` and
/// `MpvDriver` exclusively; no other task touches them.
///
/// After each event that mutates state, DaemonCore broadcasts a
/// `BroadcastMessage::StateUpdated` (or `IcyUpdated`) to all TCP clients via
...
/// only checks process liveness and the client-count grace period.
use std::sync::Arc;
...
pub enum DaemonEvent {
    /// A command received from a TCP client.
    ClientCommand(Command),
    /// A new client connected; core should broadcast Hello.
    ClientConnected,
    /// A client disconnected.  Payload is the new total client count.
    ClientCountChanged(usize),
    /// Heartbeat — check process liveness, grace-period expiry.
    HeartbeatTick,
...
pub struct DaemonCore {
    ...
    intend_playing: bool,
    /// How many clients are currently connected (for shutdown logic).
    client_count: usize,
    /// When the last client disconnected (for graceful shutdown timeout).
    ...
    /// Background ffmpeg task that calculates PCM-based audio levels.
    /// Killed when playback stops or a new station starts.
    vu_task: Option<tokio::task::JoinHandle<()>>,
}
...
impl DaemonCore {
    ...
    /// Borrow the state manager (for use by the socket server).
    pub fn state_manager(&self) -> Arc<StateManager> {
        Arc::clone(&self.state_manager)
    }

    /// Run the core event loop.  Returns when a `Shutdown` event is received
    /// or the no-client grace period expires.
    pub async fn run(mut self, mut event_rx: mpsc::Receiver<DaemonEvent>) -> anyhow::Result<()> {
        ...
        // Kick off the heartbeat ticker (replaces the old 2s poll ticker).
        // Only used for process liveness check + shutdown grace period.
        ...
        loop {
            ...
            match evt {
                ...
                Some(DaemonEvent::ClientConnected) => {
                    // Socket server already handles Hello broadcast; nothing to do here.
                }

                Some(DaemonEvent::ClientCountChanged(n)) => {
                    ...
                    if let Some(since) = self.empty_since {
                        if since.elapsed().as_secs() >= 5 {
                            info!("DaemonCore: no clients for 5s, shutting down");
                            break;
                        }
                    }
                }
    ...
    async fn handle_mpv_event(&mut self, evt: MpvEvent) {
        ...
        if let Some((obs_id, data)) = evt.as_property_change() {
            match obs_id {
                ...
                OBS_AUDIO_LEVEL => {
                    ...
                    // Parse the RMS level and broadcast it.
        ...
        match evt.event_name() {
            ...
            Some("file-loaded") => {
                ...
                // Re-register observations so mpv pushes current values immediately
                if let Some(h) = self.mpv_handle.clone() {
                    tokio::spawn(async move {
                        h.observe_all_properties().await;
                        // Re-install audio filter in case it was cleared by the new file
    ...
    async fn ensure_mpv_handle(&mut self) -> Option<MpvHandle> {
        ...
        if self.mpv_handle.is_none() {
            ...
            // Forward mpv events into our own event loop
            ...
            // Try to reconnect to an existing socket first
            let handle = match self.mpv_driver.try_reconnect(event_tx.clone()).await {
                ...
                None => {
                    // Need to spawn a fresh mpv process — create a new event channel
                    // (try_reconnect consumed the sender but never actually used it for spawning)
                    let (event_tx2, event_rx2) = mpsc::channel::<MpvEvent>(64);
                    let core_tx2 = self.mpv_event_tx.clone();
                    tokio::spawn(async move {
                        let mut rx = event_rx2;
                        while let Some(evt) = rx.recv().await {
                            if core_tx2.send(DaemonEvent::MpvEvent(evt)).await.is_err() {
                                break;
                            }
                        }
                    });
                    self.set_mpv_health(MpvHealth::Starting).await;
                    match self.mpv_driver.spawn_and_connect(event_tx2).await {
                        Ok(h) => h,
                        Err(e) => {
            ...
            tokio::spawn(async move {
                ...
                // Install the lavfi astats filter so audio levels are exposed
    ...
    // ── VU meter task (ffmpeg PCM-based audio levels) ─────────────────────────

    /// Kill any running VU meter task.
    fn stop_vu_task(&mut self) {
        if let Some(task) = self.vu_task.take() {
            task.abort();
            debug!("VU meter task stopped");
        }
    }

    /// Spawn a background task that runs `ffmpeg` on `url`, decodes the stream
    /// to mono s16le PCM at 11025 Hz, and broadcasts `AudioLevel(rms_db)` about
    /// 20× per second (every ~512 samples ≈ 46ms).
    ///
    /// ffmpeg is chosen because it already handles every protocol the daemon
    /// uses (Icecast, HLS, direct HTTP) and is available everywhere mpv is.
    fn spawn_vu_task(&mut self, url: String) {
        // Kill any previous VU task before starting a new one.
        self.stop_vu_task();

        let broadcast_tx = self.broadcast_tx.clone();

        let handle = tokio::spawn(async move {
            // Small delay so mpv gets a head-start on buffering; this avoids
            // ffmpeg and mpv racing to buffer the same stream simultaneously.
            tokio::time::sleep(tokio::time::Duration::from_millis(500)).await;

            loop {
                match run_vu_ffmpeg(&url, &broadcast_tx).await {
                    Ok(()) => {
                        // ffmpeg exited cleanly — stream ended, stop the loop.
                        info!("VU meter: ffmpeg exited cleanly");
                        break;
                    }
                    Err(e) => {
                        // Transient error (network hiccup, etc.) — retry after a pause
                        // so we don't hammer the server.
                        warn!("VU meter: ffmpeg error ({}), retrying in 3s", e);
        ...
        self.vu_task = Some(handle);
        debug!("VU meter task started");
    }
    ...
    async fn play_station(&mut self, idx: usize) -> anyhow::Result<()> {
        ...
        if let Some(station) = station {
            ...
            // Start a fresh VU meter task reading directly from the station URL.
            // This gives us PCM-based dBFS levels independent of the mpv astats filter.
            self.spawn_vu_task(station.url.clone());
...
// ── station loader (moved from main.rs) ──────────────────────────────────────
...
/// The number of mono s16le samples per RMS window.
///
/// At 11025 Hz mono, 512 samples ≈ 46ms — a good balance between
/// visual responsiveness and CPU overhead.
const VU_WINDOW_SAMPLES: usize = 512;

/// Sample rate used for ffmpeg PCM output (mono, 16-bit signed LE).
const VU_SAMPLE_RATE: u32 = 11025;

/// Run `ffmpeg -i <url> ...` indefinitely, read mono s16le PCM from stdout,
/// compute RMS dBFS for every `VU_WINDOW_SAMPLES` samples, and broadcast the
/// result via `broadcast_tx`.
///
/// Returns `Ok(())` when ffmpeg exits with code 0, `Err` on I/O or non-zero exit.
async fn run_vu_ffmpeg(
    ...
    // Sample rate as a string for ffmpeg args
    ...
    loop {
        ...
        if n == 0 {
            // EOF — ffmpeg finished (stream ended or was killed)
            break;
        }

        // Parse complete i16 samples from the bytes we received.
        let samples_in = n / 2;
        for i in 0..samples_in {
            let lo = buf[i * 2] as u8;
            let hi = buf[i * 2 + 1] as u8;
            let sample = i16::from_le_bytes([lo, hi]);
            sample_buf.push(sample);

            if sample_buf.len() >= VU_WINDOW_SAMPLES {
                let rms_db = pcm_rms_db(&sample_buf);
...
/// Calculate RMS amplitude in dBFS from a buffer of i16 samples.
///
/// Returns `-90.0` (silence floor) when RMS is 0.
